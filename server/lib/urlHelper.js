...
// i.e. time(23, 1) will return "(time like '00:%25' or time like '01:%25' or time like '02:%25')"
  ...
  // This function will pad a time to have leading 0's i.e. 06, 09, 23, 00
    var time = `${time}`;
    const padding = '00';
    const padded = padding.substring(0, padding.length - time.length) + time;
    return padded;
  }

  // Populate windowTimes to be an array of hours ['23, '00', '01', '02', '03']
  const start = time - timePeriod;
  const finish = time + timePeriod;
  ...
  let timeQuery = windowTimes.join(":%25' or time like '");
